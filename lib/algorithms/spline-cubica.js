const initialX = [0.1, 0.5, 0.9, 1.3, 1.7];
const initialY = [-2.3026, -0.69315, -0.10536, 0.26236, 0.53063];

const cubicSpline = ({ x_s, y_s }) => {
    var splines = []
    for (let i = 0; i < x_s.length; i++) {
        splines.push({ a: 0, b: 0, c: 0, d: 0, x: 0 });
    }
    buildSpline(x_s, y_s, splines)
    return {
        x_s,
        y_s,
        splines,
        interpolator: (x) => interpolate(x, splines)
    }
}

function buildSpline(x, y, splines) {
    const n = x.length;
    for (let i = 0; i < n; i++) {
        splines[i].x = x[i];
        splines[i].a = y[i];
